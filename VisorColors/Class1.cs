...
namespace VisorColors
{
    ...
    public class MoreColors : MonoBehaviour
    {
        ...
        {
            private static HashSet<BedBoss> patchedBedBosses = new HashSet<BedBoss>(); // Track patched instances.
            ...
            private static void AwakePatch(BedBoss __instance)
            {
                if (patchedBedBosses.Contains(__instance))
                {
                    ...
                    return; // Skip if this instance has already been patched.
                }

                // Copy materials and add custom colors
                ...
                {
                    Material material = UnityEngine.Object.Instantiate(oldMaterial); // Create a new material instance
                    material.color = color;
                    materials.Add(material);
                });

                __instance.materials = materials.ToArray(); // Apply updated materials list to the instance

                // Add this instance to the set of patched instances
        ...
        {
            private static HashSet<PlayerCustomizer> patchedCustomizers = new HashSet<PlayerCustomizer>(); // Track patched instances.
            ...
            private static void AwakePatch(PlayerCustomizer __instance)
            {
                ...
                // Apply modifications to this instance
                ...
                // Add custom colors, ensuring no duplicates
                List<Color> existingColors = __instance.colorsToPickFrom;
                MoreColors.Instance.colors.ForEach(color =>
                {
                    if (!existingColors.Contains(color)) // Avoid adding duplicates
                    {
        ...
        public class PlayerVisorPatch
        {
            ...
            private static void UpdatePatch(PlayerVisor __instance)
            {
                ...
                try
                {
                    ...
                    // Access the correct material (Material 2 / M_PlayerVisor)
                    ...
                    // Debugging output
                  /*Debug.Log($"Debugging, visorColorIndex = {__instance.visorColorIndex}");
                    Debug.Log($"Debugging, color = {color}");
                    Debug.Log($"Debugging, visorMaterial color = {visorMaterial.GetColor("_Color")}");*/
                }
                catch (Exception ex)
                {
